...
use imageproc::drawing::{
    draw_filled_circle, draw_filled_circle_mut, draw_line_segment_mut, draw_text_mut, Canvas,
};
...
impl<'a> Plotter<'a> {
    ...
    pub fn new(points: &'a Vec<(u128, u128)>, add_line: bool) -> Self {
    ...
    pub fn generate_image(&self, filepath: &str) -> Result<()> {
        ...
        for (x, y) in self.points.iter().map(|(x, y)| {
            (
                (*x as f64 * self.x_scale) as f32,
                ((*y - ymin) as f64 * self.y_scale) as f32,
            )
        }) {
    ...
    fn draw_linear_line(&self, img: &mut ImageBuffer<Rgb<u8>, Vec<u8>>) -> Result<()> {
        let x = (self.max_x()? - self.min_x()?) as f32 * self.x_scale as f32;
        let y = (self.max_y()? - self.min_y()?) as f32 * self.y_scale as f32;
        println!(
            "drawing line to {x},{y}, where image size is {},{}",
            img.width(),
            img.height()
        );
        draw_line_segment_mut(img, (1., 1.), (x, y), Rgb([255u8, 0u8, 0u8]));
        Ok(())
    }
    ...
    fn draw_axis(&self, img: &mut ImageBuffer<Rgb<u8>, Vec<u8>>) -> Result<()> {
        ...
        draw_line_segment_mut(
            img,
            ((img.width() - 1) as f32, (img.height() - 1) as f32),
            (1., (img.height() - 1) as f32),
            Rgb([255u8, 255u8, 255u8]),
        );
        ...
        for p in self
            ...
            .iter()
            .enumerate()
            .filter(|(i, _)| (i + 1) % (self.points.len() / 10) == 0)
            .map(|(_, v)| v)
        {
            draw_line_segment_mut(
                img,
                (
                    (p.0 as f64 * self.x_scale) as f32,
                    (img.height() - 1) as f32,
                ),
        ...
        draw_line_segment_mut(img, (0., yline), (410., yline), Rgb([64u8, 64u8, 64u8]));
    ...
    fn min_y(&self) -> Result<u128> {
        self.points
            ...
            .map(|p| p.1)
            .min()
            .ok_or(anyhow!("failed to find min/max value"))
    }
